"""
Extends gpcr_pipeline_gpr68_string_demo.py's interpolated path with a
second pH (6.0), reusing the 11 already-built path structures (pure
Cartesian interpolation -- geometrically pH-independent) rather than
rebuilding them, so only the 11 new real Gibbs calculations at pH 6.0 are
needed here (the pH 7.4 results are already in path_table.csv from the
earlier run).

Run with (~12-17 min, 11 real OpenMM minimizations at pH 6.0):

    PYTHONPATH=.:gibbs python examples/gpcr_pipeline_gpr68_string_ph6_demo.py
"""
...
from examples.gpcr_pipeline_gpr68_string_demo import OUT_DIR, N_IMAGES, run_tool3_gibbs  # noqa: E402
...
def main():
    ...
    # Overlay both pH profiles along the path
    ...
    ax.set_title(f"GPR68: Gibbs energy along the interpolation path, pH {PH_EXISTING} vs. {PH_NEW}\n(N={N_IMAGES} images)", fontsize=9)
    ...
    # Barrier height (peak - min-of-endpoints) at each pH, and how the
    # *shape* differs -- shift each curve to its own f=0 (active endpoint)
    # value so the two profiles can be compared on relative terms alongside
    # the absolute-shift plot.
    fig, ax = plt.subplots(figsize=(6.5, 4.5))
